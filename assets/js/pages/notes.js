  ...
  notesList.innerHTML = pageNotes.map(note => `
    <div class="note-card ${note.isPinned ? 'pinned' : ''}" onclick="window.viewNote('${note.id}')">
      ...
      <div class="note-actions" onclick="event.stopPropagation()">
        <button type="button" class="btn-icon" onclick="window.editNote('${note.id}')" title="Edit">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
          </svg>
        </button>
        <button type="button" class="btn-icon btn-danger" onclick="window.showDeleteConfirmation('${note.id}')" title="Delete">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor">
